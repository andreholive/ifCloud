...
export default class DiagramEngine {
  constructor(components, areShortcutsAllowed, notify, removeNotification, socket, showDeviceConfigs) {
    ...
    this.showDeviceConfigs = showDeviceConfigs;
  ...
  initializeEngine = () => {
    ...
    this.engine.notify = this.notify;
    this.engine.removeNotification = this.removeNotification;
    this.engine.commands = new CommandManager();
    this.engine.showDeviceConfigs = this.showDeviceConfigs; 
    this.engine.registerListener(commandHandlers(this));
    this.engine.getStateMachine().pushState(new States(this.socket));
    const actions = [
      DuplicateAction,
      ClipboardAction,
      DeleteAction,
  ...
  async setSockets(){
    ...
    await Promise.all(nodes.map(async node => {
      ...
      await node.functions.openListener();
    }
    ));
  }
  /**
  ...
  handleComponentDrop = (event, component) => {
   ...
   const getSnappedRelativeMousePoint = () => {
    ...
    const point = event
      ...
      : new Point(0, 0);
    const node = new Model(component.type, component.configurations, this.socket, this.engine);
    this.model.addNode(node);
    node.setPosition(point);
    node.functions.create();
  ...
  startListener(){
    let link = '';
    this.socket.on('deviceAdded', (data)=> {
      console.log('DATA', data)
      const allModels = Object.values(this.model.getModels())
      console.log(allModels)
      const model = allModels.find(
        device => device.options.id === data.configurations.id,
      );
      if(!model){
        const { Model } = this.components.find(
          c => c.type === data.device_type,
        );
        const point = new Point(data.position.x, data.position.y);
        const node = new Model(data.device_type, data.configurations, this.socket, this.engine);
        this.model.addNode(node);
        node.setPosition(point);
        node.functions.openSocket();
        this.engine.fireEvent({ nodes: [node] }, 'componentsAdded');
        this.engine.repaintCanvas();
      }
    })
    
    this.socket.on('link-create', (data) => {
      const model = this.model.getModels().find(
        device => device.options.id === data.device,
      );
      const port = model.ports[data.port]
      link = port.createLinkModel();
    ...
    this.socket.on('link-remove', (data) => {
      const model = this.model.getModels().find(
        device => device.options.id === data.device,
      );
      const link = model.ports[data.port].getMainLink();
      link.remove();
      this.engine.repaintCanvas();
    })
    
    this.socket.on('link-connect', (data) => {
      console.log(data)
      const source = this.model.getModels().find(
        device => device.options.id === data.device,
      );
      const target = this.model.getModels().find(
        device => device.options.id === data.targetDevice,
      );
      const link = source.ports[data.port].getMainLink();
      const port = target.ports[data.targetPort];
      link.setTargetPort(port);
      link.addLabel(link.sourcePort.options.name)
      link.addLabel(link.targetPort.options.name)
      this.engine.repaintCanvas();
    })
    
    this.socket.on('link-move', (data) => {
      const model = this.model.getModels().find(
        device => device.options.id === data.device,
      );
      link.getLastPoint().setPosition(data.position.x, data.position.y);
      this.engine.repaintCanvas();
    })
  }
